...
public class ResourceUtils {
    ...
    /**
     * 从 classpath 中指定路径下获取所有符合文件扩展名的资源的 InputStream，并返回文件名到 InputStream 的映射。
     * 文件名不包含扩展名。
     *
     * @param resourcePath  Classpath 资源路径（例如 "graph"）
     * @param fileExtension 目标文件的扩展名（例如 ".json"）
     ...
     * @throws IOException 如果在访问文件或 JAR 时发生 I/O 错误
     */
    public static Map<String, InputStream> getResourceInputStreamsMap(String resourcePath, String fileExtension)
            throws IOException {
        Map<String, InputStream> inputStreamsMap = new HashMap<>();
        ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
        if (classLoader == null) {
            classLoader = ResourceUtils.class.getClassLoader();
        }
            ...
            if (resourceUrl.getProtocol().equals("file")) {
                ...
                if (folder.exists() && folder.isDirectory()) {
                    ...
                    if (files != null) {
                        for (File file : files) {
                            ...
                            inputStreamsMap.put(fileNameWithoutExtension, new java.io.FileInputStream(file));
                        }
                    }
                } else {
                    log.warn("ResourceUtils: File URL {} is not an existing directory. Skipping.", resourceUrl);
                ...
                try (JarFile jarFile = new JarFile(decodedJarFilePath)) {
                    ...
                    while (entries.hasMoreElements()) {
                        ...
                        if (entryName.startsWith(resourcePath + "/") && entryName.endsWith(fileExtension)) {
                            log.debug("ResourceUtils: Found entry in JAR: {}", entryName);
                            String fileNameWithExtension = entryName.substring(entryName.lastIndexOf('/') + 1);
                            String fileNameWithoutExtension = fileNameWithExtension.substring(0, fileNameWithExtension.lastIndexOf('.'));
                            inputStreamsMap.put(fileNameWithoutExtension, new java.io.ByteArrayInputStream(jarFile.getInputStream(entry).readAllBytes()));
                        }
                    }
                } catch (IOException e) {
                    log.error("ResourceUtils: Error opening or reading JAR file {}: {}", decodedJarFilePath,
