...
public abstract class BaseFlushExtension<T> extends BaseExtension {
    ...
    protected FlowableProcessor<Flowable<T>> streamProcessor = UnicastProcessor.<Flowable<T>>create()
        .toSerialized();
    ...
    protected Disposable generateDisposable(TenEnv env) {
        return streamProcessor
            ...
            .concatMap(flowable -> flowable
                ...
                // 中断检测，flush后中断当前流处理
                .takeWhile(item -> !interrupted.get()))
            .subscribe(
                item -> handleStreamItem(item, env),
    ...
    public synchronized void flushInputItems(TenEnv env, Command command) {
        ...
        // 重新创建流发布器（线程安全的）
        streamProcessor = UnicastProcessor.<Flowable<T>>create().toSerialized();
     ...
     */
    protected abstract void handleStreamItem(T item, TenEnv env);
